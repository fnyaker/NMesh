...
| `pkg_dir.py` | **The package directory**: a signed record saying what one key publishes, filed under the publisher id and under the package name's prefixes — so "what does this node offer?" and "who publishes something called this?" are the same lookup with two keys. Also the source digest that answers "do these publishers agree on the *code*?" without downloading anything, and the two-halved **pairing** that ties a detached publisher key to the machine using it — neither party can sign the other's half, so one alone is never followed. |
| `subscriptions.py` | What this node watches for a new version, and how many watched publishers must agree on the code before one installs itself. Local, deliberate, never writable from the network — like the pins beside it. |
