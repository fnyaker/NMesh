...
| `transports/` (`contract.py`, `medium.py`, `manager.py`, `registry.py`, `tcp.py`, `udp.py`, `spool.py`, `bundle.py`) | **The transports**: the `BaseTransport`/`BaseServer` interfaces, the one place the core asks a medium a question (and checks the answer), a registry by URL scheme, the one file naming the schemes the node ships (`registry.py`), and the three media written to be read. The old flat paths (`transport.py`, `udp_transport.py`, …) are permanent aliases. |
...
7. **[logging.md](logging.md)** — the log engine: why nothing is kept by
   ...
   gap, the two unequal halves of an app's access to it, and the merged **live
   tail** a console watching the whole network follows.
