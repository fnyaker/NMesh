...
| `subscriptions.py` | What this node watches for a new version — **a package**, never a publisher — and how many endorsed signing keys must agree on the code before one installs itself. Local, deliberate, never writable from the network — like the pins beside it. |
