...
class TestTermux:
    """Android/Termux: no init system, no root, no sudo — and an exported
    PREFIX environment variable that collides with the installer's own."""

    def test_termux_is_detected_by_its_prefix(self, tmp_path):
        tprefix = tmp_path / "termux"
        (tprefix / "bin").mkdir(parents=True)
        (tprefix / "bin" / "pkg").write_text("#!/bin/sh\nexit 0\n")
        (tprefix / "bin" / "pkg").chmod(0o755)
        result = run_snippet(tmp_path, "detect_init",
                             env={"PREFIX": str(tprefix)}, isolate=True)
        assert result.stdout.strip() == "termux"

    def test_termux_wins_over_a_stray_systemctl(self, tmp_path):
        """A proot-distro image can carry a systemctl binary: the PREFIX probe
        must decide first, or the node would get a unit nothing ever loads."""
        tprefix = tmp_path / "termux"
        (tprefix / "bin").mkdir(parents=True)
        (tprefix / "bin" / "pkg").write_text("#!/bin/sh\nexit 0\n")
        (tprefix / "bin" / "pkg").chmod(0o755)
    ...
    def test_the_service_loop_runs_start_sh_and_restarts_it(self, tmp_path):
        result = run_snippet(
            tmp_path, 'termux_service /t/nmesh /t/nmesh/data "--fleet" /termux')
        out = result.stdout
        assert out.startswith("#!/termux/bin/sh")
        assert 'export NMESH_DATA="/t/nmesh/data"' in out
        assert 'export HOME="/t/nmesh"' in out
        assert 'export OQS_INSTALL_PATH="/t/nmesh/_oqs"' in out
        # The updater only restarts a node something brings back: this loop.
        assert 'export NMESH_SERVICE_MANAGED=1' in out
        # start.sh recognises Android by Termux's own PREFIX, not the installer's.
        assert 'export PREFIX="/termux"' in out
        assert '"/t/nmesh/start.sh" --fleet' in out
        assert "sleep 5" in out

    def test_the_log_stays_bounded(self, tmp_path):
        out = run_snippet(
            tmp_path, 'termux_service /t/nmesh /t/nmesh/data "" /termux').stdout
        assert "service.log" in out
        assert "1048576" in out

    def test_the_boot_hook_backgrounds_the_loop(self, tmp_path):
        """Termux:Boot runs the scripts of ~/.termux/boot one after another: a
        loop that never exits must not block the ones behind it."""
        out = run_snippet(tmp_path, "termux_boot_hook /t/nmesh /termux").stdout
        assert '"/t/nmesh/termux-service.sh"' in out
        assert out.rstrip().endswith("&")

    def test_the_boot_hook_lives_under_dot_termux_boot(self):
        assert ".termux/boot/$SERVICE" in INSTALL.read_text()

    def test_the_supervisor_dies_before_the_node(self):
        """Kill the node while its loop lives and it comes straight back —
        behind the uninstaller's back."""
        text = INSTALL.read_text()
        assert text.index('pkill -f "$1/termux-service.sh"') \
            < text.index('pkill -f "nmesh_node.py --data $2"')

    def test_locking_down_as_oneself_needs_no_escalation(self, tmp_path):
        """Where there is no sudo at all, a chown to oneself is a no-op that
        used to *fail* the whole install."""
        target = tmp_path / "tree"
        target.mkdir()
        result = run_snippet(
            tmp_path,
            f'SUDO=none; RUN_USER="$(id -un)"; OWNER=x; lock_down "{target}" && echo LOCKED')
