  ...
  <main>
    ...
    <section id="panel-settings" class="panel" role="tabpanel" data-panel="settings" hidden>
      ...
      <article class="surface">
        <div class="surface-head"><div><p class="eyebrow">Software</p><h2>Updates</h2></div><span id="version-pill" class="state-pill"></span></div>
        <p class="subtle">Checks the published releases of this project on GitHub. Nothing is ever installed
          without you confirming the exact version — and the node restarts to run it.</p>
        <div class="action-row">
          <button id="update-check" class="secondary">Check for updates</button>
          <button id="update-apply" class="primary" hidden>Install</button>
        </div>
        <p id="update-status" class="subtle"></p>
...
let UPDATE_OFFER = null;      // the exact release the operator is looking at

function renderVersion(state){
  const pill=$("version-pill");
  if(pill&&state&&state.version)pill.textContent="v"+state.version;
}
async function checkForUpdates(){
  const button=$("update-check"),status=$("update-status"),notes=$("update-notes");
  button.disabled=true; status.textContent="Asking GitHub…";
  $("update-apply").hidden=true; notes.hidden=true; UPDATE_OFFER=null;
  try{
    const data=await(await api("/api/update/check")).json();
    ...
    if(!data.can_apply){
      status.textContent=data.latest+" is available, but this install cannot update itself: "+data.blocked;
      return;
    }
    ...
    apply.textContent="Install "+data.latest; apply.hidden=false;
...
async function applyUpdate(){
  ...
  if(!confirm("Install "+UPDATE_OFFER+"? The node will replace its own files and restart."))return;
  const button=$("update-apply"),status=$("update-status");
  button.disabled=true; status.textContent="Downloading and installing "+UPDATE_OFFER+"…";
  try{
    const response=await api("/api/update/apply","POST",{version:UPDATE_OFFER,confirm:true});
    const data=await response.json().catch(()=>({}));
    ...
    status.textContent=data.service_managed
      ...
      : ("Installed "+data.applied+". Restart the node to run it (the previous files are in "+data.backup+").");
    button.hidden=true; UPDATE_OFFER=null;
